...
> Heb datum dynamisch gemaakt

> Vul jij titel nog aan

> Je moet nog heel refactoren om werkende te krijgen: Code chunk namen in "chunk-name" formaat (geen hoofdletters, geen underscores, geen spaties). Je kan met `Ctrl + Down` snel van chunk naar chunk/heading lopen. Dit maakt het moeilijk om te reviewen. Algemeen drijg je punten aan slordigheid te verliezen.

> Best met Titels beginnen om skelet van dit document goed te krijgen
...
> Probeer voldoende te kaderen: This report describes the results of a research to predict emotion from tweets. More in particular...

The goal of this project is to predict an emoji based on text.

> 'They' moet je hier specifiëren (naam onderzoeker, tijdstip, url)

When looking for similar projects, they get to an accuracy of 33%, this will be the succes criterion
...
> Moet hier niet per sé staan
Use this data set if you want to check or develop some steps really quickly
...
> In principe dataset niet overschrijven met nieuwe
...
so it is added together and split later.

```{r load full dataset, eval=TRUE}
tweetDT=rbind(fread("./data/train.csv"),fread("./data/test.csv"))
```

> Best om tab-delimited bestanden te gebruiken ipv comma-separated!
...
> Gebruik dt waarvoor ze dienen: `tweetDT[, text := text %>% tolower]`

> Maak uw code een beetje leesbaarder met `Ctrl = Shift + A`
...
The eventual decided length is 20 words per tweet since most tweets have few words and longer tweet size requires more computation.
...
```{r tweet length plot}
tweetDT$text %>% strsplit(" ") %>% sapply(length) %>% v
```

> Vermijd dubbel werk! Gebruik variabelen + Werk code af
...
In the ideal situation we would have 6170786/49 = 125,934 tweets per emoji

> Waarom 'ideal', balanced?

The most popular emoji 'face_with_tears_of_joy' has over a million tweets.

The least popular emoji 'heavy_check_mark' has only 23.5 thousand tweets.

> Ter info: ideaal om hier met in-line R code te werken zodat deze cijfers ook nog kloppen met het updaten van brondata

The Median is lower than the Mean, this means that most tweets have fewer emoji's than the average.

> Verdeling tonen: `tweetDT$emoji %>% table %>% sort %>% rev %>% barplot`

```{r average number of tweets per emoji}
...
> Lege haakjes `()` niet nodig tijdens chaining

Here we see that some emoji's have a lot more tweets, but most emoji's have a proportionate amount

```{r Distribution of tweets per emoji}
emoji_freq$Freq%>%sort()%>%rev()%>%plot(main="Distribution of tweets per emoji") 
```
...
> Vraagt voor tabel: |Word count|Model Size|Duration|Accuracy|
...
Here you can see a sample of the vocabulary used by the text vectorization layer.

```{r the used vocabulary, eval=FALSE}
get_vocabulary(text_vectorization)%>%sample(100)
```

> Hier krijg ik error "'str' object has no attribute 'decode'"

Here is shown how the text vectorization layer transforms the input

```{r how the text vectorization transforms the input}
tweetDT$text[7]
...
- num words = 10000
- layer size = 192(input),192(dense),192(gru)

NO stemming

- 10.5 minutes / epoch for 15 epochs
- accuracy 27.82%

WITH stemming

- 9.8 minutes / epoch for 15 epochs
- accuracy 27.82%
...
> Bij zwaar werk best switch maken `if (!file.exists("history.RDS")){do work}else{load}`
...
> Structuur met titels ipv text maakt dat je erg veel onderhoudswerk hebt, houd hier een logische Notebook stijl aan
...
> Moet je even herbekijken, staat hier een beetje vreemd
...
> print wordt zelden in Notebook gebruikt.
...
Analog to the words we examine how many 3-grams there are per tweet.

```{r tweet length summary2
tweetDT$V2 %>% strsplit(" ") %>% sapply(length) %>% summary()
```

> Hier loopt het wat mis, probeer de Notebook structuur te omarmen ipv er tegen te vechten. De structuur zet namelijk aan om het proces van functionele scripting te structureren: denk - doe - faal - denk - doe - denk - doe... ipv doe-doe-doe-faal-doe-doe-doe-doe-faal-denk

```{r tweet length plot2}
...
### create 3-grams as one hot matrix

```{r ngrams as one hot2}
sampleDT = tweetDT[1:1000]
...
```{r ngrams as one hot}
ngrams <- tokens(tweetDT$text,"character")%>% tokens_ngrams(n=3,concatenator='')
...
clusterExport(cl, "ngrams")

oh<- do.call(rbind, parLapply(cl,ngrams, function(i) as.integer(!is.na(match(unique(unlist(ngrams)), i)))))

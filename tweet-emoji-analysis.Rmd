...
output <- input %>% 
  ...
  #layer_global_average_pooling_1d() %>%
  layer_lstm(units = 49,activation = "relu",dropout=0.2, recurrent_dropout=0.2)
  layer_dense(units = emoji_count, activation = "softmax")
...
output <- input %>% 
  ...
  layer_embedding(input_dim = num_words + 1, output_dim = 40) %>%
  layer_spatial_dropout_1d(0.2)%>%
  layer_gru(units = 45,dropout=0.4, recurrent_dropout=0.4)%>%
  layer_dense(units = emoji_count, activation = "softmax")

...
The data is gathered, selected and reformatted by 

>  [emojifydata](https://www.kaggle.com/rexhaif/emojifydata-en) _Daniil Larionov_ from the _ArchiveTeam Twitter data_ source (2019)
...
The source .txt data is converted to .tsv files which have been manually compressed to gzip format.
...
This set is included in the project files.
...
This set is not included in the project files since it is too large.

It can be generated with the _tweet-pre-process.ipynb_ notebook and the source data.
...
The most popular emoji face_with_tears_of_joy has 1009912 tweets.

The least popular emoji heavy_check_mark has only 23539 tweets.
...
```{r adapt-text-vectorization-layer, warning=FALSE, message=FALSE}
num_words <- 10000
max_length <- 20
...
#### Comparing max tweet length variations
...
example_id <- sample.int( nrow( tweetDT ), 1 )

tweetDT$text[ example_id ]

text_vectorization( matrix( tweetDT$text[ example_id ], ncol = 1))
```
...
we suffered from so called data leakage.
...
RNN's model the autocorrelation in a sequence by implementing a type of working memory, the cell state.
...
GRU's are similar too but less complex than LSTM's which makes them, in this case, twice as fast. 
...
| 10000 | 192 | 192(dense), 192, 192| 12 | +15 | 27.88% |

It appears that placing a dense layer before the GRU layer increases the accuracy.

```{r best-one-GRU-layer-model, warning = FALSE, eval = FALSE}
input <- layer_input( shape = c( 1 ), dtype = "string" )
...
Adding a second GRU layer increases the accuracy some more.
...
output <- input %>% 
  ...
  layer_dense( units = 192 ) %>%
  layer_dropout( 0.4 ) %>%
  layer_gru( units = 192, return_sequences = TRUE ) %>%
  layer_dropout( 0.5 ) %>% 
  layer_gru( units = 192 ) %>%
  layer_dropout( 0.5 ) %>% 
  layer_dense( units = emoji_count, activation = "softmax" )
...
```{r bidirectional-GRU, eval=FALSE}
input <- layer_input( shape = c( 1 ), dtype = "string" )
...
train <- function() {
  ...
  model %>% save_model_tf( "files/my_model" )
  saveRDS( history, 'files/learn-history.rds' )
}

if( file.exists( "files/my_model" )){
  tryCatch( model <- load_model_tf( "files/my_model" ) , error = function( e ){ train() } )
  
}else { train() }
```

```{r plot-training-history}
if( file.exists( 'files/learn-history.rds' ) ){
  history <- readRDS( 'files/learn-history.rds' )
  plot( history )
}
```
...
```{r evaluate-model}
if( file.exists( 'files/evaluation.rds' ) ){
  results <- readRDS( 'files/evaluation.rds' )
  print( results )
  
}else{
  results <- model %>% evaluate( testing$text, oh_test, verbose = 0 )
  print( results )
  saveRDS( results, 'files/evaluation.rds' )
}
```
...
### Checking the effect of stemming

Let's try stemming as a technique to reduce the total amount of different words.

Stemming reduces the total amount of different words from 613,993 words to 529,416.
...
smpl_to_stem <- sample.int( nrow( tweetDT ), 10 )
tweetDT_smpl <- tweetDT[ smpl_to_stem, ]

print( tweetDT_smpl )

stemmedtext <- stemDocument( tweetDT_smpl$text )
...
gram <- tokens( tweetDT_subset$text, "character" ) %>% 
...
```{r text-vectorization-layer-3-grams,eval=FALSE}
num_words <- 10000
max_length <- 100
text_vectorization <- layer_text_vectorization(
  max_tokens = num_words, 
...
```{r ngrams-as-one-hot}
ngrams <- tokens( sampleDT$text, "character" ) %>% 
  tokens_ngrams( n=3, concatenator='' )
...
oh <- do.call( rbind, lapply( ngrams, function( i ) X = as.integer( !is.na( match( unique( unlist( ngrams )), i )))))
```
...
This part describes a workflow of how you could create a model with the 3-grams as a one hot encoded matrix as input.
...
```{r compile-ngram-model,eval=TRUE}
model %>% compile(
  optimizer = 'adam',
  loss = 'categorical_crossentropy',
  metrics = list( 'accuracy' )
...
These results are not important and are just a proof of concept.
...
The initial goal was to reach an accuracy of 33%, the highest accuracy we achieved was 27.91% on this data set, this means that the official criterion has not been met. 
...
We found that GRU networks are the best architecture for this particular problem. We get the advantage of recurrent neural networks without the complexity of LSTM's. 

The model can suggest some emojis based on any piece of text, the suggestions seem plausible enough, 

and I learned a ton in this project so I see this as a total win.

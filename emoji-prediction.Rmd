...
```{r eval=TRUE}
trainDT <- fread("./data/train.csv")
testDT <- fread("./data/test.csv")
...
- to lower case
- remove empty tweets (tweets that were only non ascii and numbers)
...
```{r}
tweetDT$text %>% strsplit(" ") %>% sapply(length) %>% hist(breaks=55) 
```
...
prediction <- predict(new_model,"hello")[1,]
#prediction <- predict(new_model,testing$text[id])[1,]
...
## tweet sentiment emoji correlation

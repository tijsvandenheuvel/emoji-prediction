...
```{r text-vectorization-layer-3-grams ,include=FALSE,cache=TRUE}
num_words <- 10000
max_length <- 100
text_vectorization <- layer_text_vectorization(
  max_tokens = num_words, 
...
Here we demonstrate a proof of concept but the entire data set would generate a sparse 0-1 matrix of `nrow(tweetDT)` x ~17000 dimensions, which my computer couldn't handle at all.
...
```{r the-used-vocabulary2, eval=FALSE}
get_vocabulary(text_vectorization)%>%sample(100)
```

```{r split-data-in-test-and-training-set2}
training_id <- sample.int(nrow(tweetDT), size = nrow(tweetDT)*0.8)
training <- tweetDT[training_id,]
testing <- tweetDT[-training_id,]
oh_train <-one_hot_emoji[training_id,]
...
```{r best-model2, message=TRUE, warning=FALSE}
input <- layer_input(shape = c(1), dtype = "string")
